...
@Component({ standalone: true, selector: 'app-profile-notification-preferences', imports: [MatIconModule, MatTooltipModule], templateUrl: './profile-notification-preferences.component.html', changeDetection: ChangeDetectionStrategy.Eager,
...
export class ProfileNotificationPreferencesComponent implements OnInit {
    ...
    ngOnInit(): void { this.pushActive = this.push.isEnabled(this.session.userId); this.load(); }
    ...
    save(): void {
        ...
        this.saving = true;
        this.notifications.savePreferences(this.preferences).subscribe({ next: () => { if (!this.preferences.some(item => item.Canal === 'push' && item.Habilitado)) this.push.revoke(this.session.userId).subscribe(() => this.pushActive = false); this.saving = false; this.toasts.showSuccess('Preferencias de notificaciones guardadas.', { title: 'Notificaciones actualizadas', dedupeKey: 'preferences:notifications:save' }); }, error: error => { this.saving = false; this.observeQaSaveError(error); const cause = getProductStateMessage(error, 'La API no ha aceptado los cambios.'); this.toasts.showError(cause, { title: 'No se guardaron las notificaciones', dedupeKey: 'preferences:notifications:save:error' }); } });
    }
    enablePush(): void { if (this.activatingPush || this.pushActive) return; this.activatingPush = true; this.push.enable(this.session.userId).subscribe({ next: () => { this.activatingPush = false; this.pushActive = true; this.toasts.showSuccess('Push activado en este dispositivo.', { title: 'Notificaciones Push activadas', dedupeKey: 'preferences:push:enable' }); }, error: () => { this.activatingPush = false; this.toasts.showError('No se ha podido activar push.', { title: 'No se pudieron activar las notificaciones Push', dedupeKey: 'preferences:push:enable:error' }); } }); }
    ...
    private observeQaSaveError(error: unknown): void { if (environment.environmentName === 'qa') sessionStorage.setItem('qa:last-notification-save-error', getApiErrorCode(error) ?? String((error as { status?: unknown })?.status ?? 'unknown')); }
}
