...
export class ProfileNotificationPreferencesComponent implements OnInit {
    ...
    load(): void { this.loading = true; this.notifications.preferences().subscribe({ next: value => { this.preferences = this.normalize(value); this.loading = false; }, error: () => { this.loading = false; this.toasts.showError('No se han podido cargar las preferencias de notificaciones.'); } }); }
    ...
    save(): void {
        ...
        this.saving = true;
        this.notifications.savePreferences(this.preferences).subscribe({ next: () => { if (!this.preferences.some(item => item.Canal === 'push' && item.Habilitado)) this.push.revoke(this.session.userId).subscribe(); this.saving = false; this.toasts.showSuccess('Preferencias de notificaciones guardadas.'); }, error: () => { this.saving = false; this.toasts.showError('No se han podido guardar las preferencias de notificaciones.'); } });
    }
    enablePush(): void { if (this.activatingPush) return; this.activatingPush = true; this.push.enable(this.session.userId).subscribe({ next: () => { this.activatingPush = false; this.toasts.showSuccess('Push activado en este dispositivo.'); }, error: () => { this.activatingPush = false; this.toasts.showError('No se ha podido activar push.'); } }); }
