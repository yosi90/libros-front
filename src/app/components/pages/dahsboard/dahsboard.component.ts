...
export class DahsboardComponent implements OnInit {
    ...
    handleEnterEvent(event: KeyboardEvent) {
        if (this.modPassword === true && this.fgPassword.valid)
            this.updatePassword();
        else if (this.newBook === true && this.fgBook.valid)
            this.addBook();
    }
    ...
    updatePasswordNewErrorMessage() {
        if (this.passwordNew.hasError('required'))
            this.errorPasswordNewMessage = 'La contraseña nueva no puede quedar vacia';
        ...
        else if ((this.passwordNew.value != this.passwordRepeat.value) && this.passwordRepeat.value) {
            ...
            this.errorPasswordNewMessage = 'La contraseña nueva debe ser distinta a la anterior';
            this.passwordNew.setErrors({ 'La contraseña nueva debe ser distinta a la anterior': true })
    ...
    updatePasswordRepeatErrorMessage() {
        if (this.passwordRepeat.hasError('required'))
            ...
            this.errorPasswordRepeatMessage = 'La contraseña nueva debe ser distinta a la anterior';
            this.passwordRepeat.setErrors({ 'La contraseña nueva debe ser distinta a la anterior': true })
    ...
    updatePassword(): void {
        ...
        const token = this.loginSrv.token;
        this.userSrv.updatePassword(this.fgPassword.value.passwordNew ?? '', this.fgPassword.value.passwordOld ?? '', token).subscribe({
            next: (user) => {
                this.userData = user;
                this.modPassword = !this.modPassword;
