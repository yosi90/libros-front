...
- El workflow manual se fusionará con `QA_HOSTING_DEPLOY_ENABLED=false` para verificar primero WIF sin desplegar. Tras esa comprobación, solo el propietario podrá habilitar el flag para ejecutar la campaña Playwright web y el despliegue acordados.
...
- Marcar el PR frontend #1 como listo para revisión y fusionarlo únicamente tras aprobación, conservando `QA_HOSTING_DEPLOY_ENABLED=false`.
