...
| QA-15-004 | Alta, backend QA | `GET /admin/backup` autoriza correctamente, pero el administrador continúa recibiendo `500` aunque OpenAPI declara `409 admin_backup_unavailable_in_qa`. | Reabierto en `docs/peticiones/corregir-backup-administrativo-qa.md`: la campaña `32951926027` y `/verify` acreditan que la release limpia `16090b4c…` aún no cumple el contrato. No se relaja la aserción. |
