...
- **Autenticación Mobile adaptable:** `MobileAuthPageComponent` admite ahora, mediante custom properties con fallback, ajustar separación de columnas y alineación del panel sin duplicar su grid ni afectar a las demás rutas. La superficie fullscreen de correo/teléfono permanece local al Login mientras no exista un segundo consumidor; el atributo funcional `data-native-back-overlay` sí queda como contrato reutilizable del runtime Android.
