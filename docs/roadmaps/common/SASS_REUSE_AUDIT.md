...
- **Campos Material narrativos Mobile:** capítulo y formularios de entidades comparten los mixins sin emisión `mobile-outlined-field-tokens` y `mobile-option-panel` en `_primitives.sass`. Centralizan tinta, bordes, foco, caret, iconos y paneles light/dark sin convertir la composición de ambos formularios en una primitive común.
