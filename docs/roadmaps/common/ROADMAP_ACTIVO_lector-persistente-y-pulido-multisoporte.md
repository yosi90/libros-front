...
- [ ] **Shell, navegación y biblioteca/catálogo**
  ...
  - [ ] **Catálogo Mobile:** retirar el encabezado interno redundante, reducir filtros a icono sin borde y llevar la solicitud de título a la misma fila. Cuando una búsqueda no devuelva resultados, la solicitud se convierte en la acción principal a ancho completo; las cards deben mantener títulos legibles en light y dark.
    ...
    - **Validación técnica:** `MobileScopedSearchComponent` sirve a Biblioteca y Catálogo; el catálogo filtra localmente nombre y autores sobre el resultado canónico para conservar ámbitos y normalización sin prometer universo/saga, ausentes en su respuesta. El panel Mobile queda anclado, acotado entre barras y por encima de las superficies ordinarias. `@capacitor/status-bar` sincroniza fondo y contraste nativos. Build web y nativa QA, 363 unitarias, 34 controles, typecheck E2E, cinco visuales Mobile y compilación `assembleQaDebug` quedan verdes; la release local se detiene correctamente al no disponer de secretos de firma. La campaña publicada de `1.0.40-qa` detectó que el panel compact aún usaba la antigua cota fija de 56 px y solapaba 11–12 px de la app bar de 64 px en Chromium y Firefox. El hotfix deriva ahora su cota de `--mobile-appbar-height` y `--app-safe-top`; la aserción publicada conserva el contrato estricto de no solape. Queda pendiente repetir esa integración.
