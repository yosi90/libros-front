...
- [ ] **Shell, navegación y biblioteca/catálogo**
  ...
  - [ ] **Biblioteca y chrome autenticado:** retirar de la vista los títulos y el contador ya expresados por la app bar, reducir filtros a un icono sin borde y corregir la composición medium. El rail debe comenzar bajo app bar y safe area; campana y perfil deben compartir cota y objetivo de 48 px; el menú Más debe presentarse como panel inferior centrado, con dos columnas, sin scroll y con tirador operativo.
    - **Validación técnica:** la biblioteca conserva los contadores de cada agrupación y elimina únicamente el resumen duplicado. El rail usa la misma reserva superior que el contenido; campana y perfil comparten geometría; el panel Más se centra en el área útil a la derecha del rail y su tirador admite pulsación y arrastre descendente con umbral de 72 px. La regresión unitaria del gesto y la integración autenticada medium fijan estos contratos. `1.0.28-qa` confirmó físicamente la composición desplegada, las dos columnas, la ausencia de scroll y el cierre mediante arrastre; también reveló que el tirador decorativo global seguía apareciendo bajo el control real. Se elimina con un selector específico y se añade al contrato E2E antes de repetir la inspección.
