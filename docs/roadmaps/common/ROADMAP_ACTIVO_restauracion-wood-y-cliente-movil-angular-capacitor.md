...
- [ ] **Hito 7 - Crear el shell, la zona pública y la autenticación Mobile.**
  - **Estado:** en progreso desde el 28 de agosto de 2026. El selector seguro, el shell público y las vistas independientes Home/Login ya están implementados. Capacitor fuerza Mobile; navegador solo lo activa con la feature flag y admite una previsualización limitada a `localhost`/`127.0.0.1` mediante `book-front:mobile-presentation-preview`. Wood continúa como fallback y sus baselines públicos permanecen idénticos. Restan registro, onboarding, recuperación, verificación y App Links; producción y QA mantienen la feature flag apagada.
