...
- [ ] **Hito 13 - Migrar la autenticación completa a Firebase y sincronizar preferencias.**
  ...
  - **Avance operativo:** backend corrigió el vínculo telefónico en la release `16090b4ce05eda9307da29679bdfc9cb6e1616ee` y la campaña completa `32734302486` quedó verde. El smoke Google real completó OAuth y normas, pero Firefox mostró `[DEFAULT]` porque seguía ejecutando `chunk-OK5TS2FP.js`, ausente del servidor, desde un Angular Service Worker huérfano; la corrección `libros-canonical-session` sí estaba desplegada en el chunk actual. La causa era que `build:qa` no publicaba `ngsw.json`. QA genera ahora un artefacto optimizado con output hashing, manifiesto y worker; Hosting sirve shell/manifiesto sin caché y el workflow verifica las tres garantías antes de desplegar. Build QA, barrera PWA, 21 controles y typecheck E2E están verdes. Falta desplegar esta transición, confirmar registro del worker en Hosting y repetir Google. El Hito 13 y producción permanecen sin aprobación hasta entonces.
