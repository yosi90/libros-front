...
- [ ] **Hito 13 - Migrar la autenticación completa a Firebase y sincronizar preferencias.**
  ...
  - **Avance operativo:** la campaña alojada `32729709020` acredita matriz de cinco perfiles, realtime Chromium/Firefox, Hosting, smoke público, restauración Strict de las cuatro identidades password, cleanup `baseline` y evidencia sanitizada. Su único bloqueo reproducible fue teléfono, corregido por backend en la release `16090b4ce05eda9307da29679bdfc9cb6e1616ee` y el dataset `2026.08.4`; la validación focal `32733090045` confirma el arreglo con `2/2` recorridos telefónicos aislados verdes. La campaña total `32733418202` superó gate, build y los escenarios ejecutados, pero el keepalive coincidió con un reset de Firefox y confundió `ContinueCampaign=blocked` más la única razón `reset_in_progress` con un bloqueo definitivo. El cleanup quedó verde. Debe corregirse esta carrera para que espere exclusivamente ese estado transitorio y siga bloqueando cualquier otra razón; después se repetirá la campaña completa y quedará únicamente el smoke Google manual. Los subhitos 13.0–13.4 están implementados, pero 13.5, el Hito 13 y producción permanecen sin aprobación explícita.
