...
- [ ] **Hito 13 - Migrar la autenticación completa a Firebase y sincronizar preferencias.**
  ...
  - **Avance operativo:** backend corrigió el vínculo telefónico en la release `16090b4ce05eda9307da29679bdfc9cb6e1616ee` y la campaña completa `32734302486` quedó verde. El smoke Google real completó OAuth y normas, pero Firefox mostró `[DEFAULT]` porque un Angular Service Worker huérfano servía un chunk ausente; la corrección `libros-canonical-session` sí estaba desplegada. QA genera ahora hashes, manifiesto y worker, añade cabeceras y barreras local/alojada. El run `32740929813` superó gate, integración local, despliegue, smoke PWA alojado y login telefónico; Firefox falló después al recibir desde el worker dos fondos wood truncados o con codificación inválida, y el cleanup quedó verde. Fondos, animaciones pesadas y recursos descartados se excluyen del cache PWA, conservando shell, fuentes, iconos y recursos funcionales pequeños, con una barrera de artefacto que impide la regresión. El Hito 13 y producción permanecen sin aprobación.
