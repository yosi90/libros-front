...
- [ ] **Hito 13 - Migrar la autenticación completa a Firebase y sincronizar preferencias.**
  ...
  - **Avance operativo:** backend corrigió el vínculo telefónico en la release `16090b4ce05eda9307da29679bdfc9cb6e1616ee`; la campaña automática completa `32746025039` quedó verde y el smoke Google real completó OAuth. Las pestañas que aún conservaban el worker defectuoso anterior requirieron `Ctrl+F5` una sola vez; con la versión actual activa, una comprobación en navegador controlado por `ngsw-worker.js` confirma que `/__/auth/handler` entrega el handler Firebase y nunca la SPA. Esta comprobación queda como regresión alojada y cerrar el popup libera el loader. El propietario autorizó producción y se emitió `docs/peticiones/autorizar-corte-productivo-autenticacion-firebase.md`; queda recibir la respuesta backend y ejecutar un smoke productivo no destructivo.
