...
- [ ] **Hito 13 - Migrar la autenticación completa a Firebase y sincronizar preferencias.**
  ...
  - **Avance operativo:** la campaña alojada `32729709020` acredita matriz de cinco perfiles, realtime Chromium/Firefox, Hosting, smoke público, restauración Strict de las cuatro identidades password, cleanup `baseline` y evidencia sanitizada. Su único bloqueo reproducible fue teléfono: preflight `201` y Firebase Phone Auth correctos terminaban en `403 phone_access_method_not_linked`. Backend respondió con la release `16090b4ce05eda9307da29679bdfc9cb6e1616ee` y el dataset `2026.08.4`, añadió `auth.phone.member-a`, fijó `libros-auth:phone:900003` y restauró su vínculo HMAC con `user.member-a`. La validación focal `32733090045` confirma el arreglo con `2/2` recorridos telefónicos aislados verdes en Chromium y Firefox, seguida de `baseline`, lease liberada y evidencia sanitizada; la petición queda aceptada y cerrada en `docs/peticiones/respondidas/ACEPTADA_restaurar-vinculo-telefono-ficticio-qa.md`. Corresponde repetir la campaña completa y después quedará únicamente el smoke Google manual. Los subhitos 13.0–13.4 están implementados, pero 13.5, el Hito 13 y producción permanecen sin aprobación explícita.
