...
- [ ] **Hito 13 - Migrar la autenticación completa a Firebase y sincronizar preferencias.**
  ...
  - **Avance operativo:** backend corrigió el vínculo telefónico en la release `16090b4ce05eda9307da29679bdfc9cb6e1616ee` y la campaña completa `32734302486` quedó verde. El smoke Google real completó OAuth y normas, pero Firefox mostró `[DEFAULT]` porque un Angular Service Worker huérfano servía un chunk ausente; la corrección `libros-canonical-session` sí estaba desplegada. QA genera ahora hashes, manifiesto y worker, añade cabeceras y una barrera previa. El run `32737918743` superó esa barrera, pero no desplegó: el worker se registró también en `127.0.0.1` e interfirió con el reset realtime de Firefox (`NS_ERROR_INVALID_CONTENT_ENCODING`); cleanup quedó verde. El registro QA se ha restringido después al Hosting canónico y una barrera Playwright comprueba su ausencia en la integración local; 244 unitarias, build QA, artefacto PWA, typecheck y smoke público Firefox local quedan verdes antes de repetir. El Hito 13 y producción permanecen sin aprobación.
