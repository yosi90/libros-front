...
- [ ] **Hito 13 - Migrar la autenticación completa a Firebase y sincronizar preferencias.**
  ...
  - **Avance operativo:** backend corrigió el vínculo telefónico en la release `16090b4ce05eda9307da29679bdfc9cb6e1616ee` y la campaña automática completa `32742203290` quedó verde. El smoke Google real posterior reveló que el Angular Service Worker trataba `/__/auth/handler` como navegación SPA y abría `/home` dentro del popup; además, cerrar esa ventana podía dejar el loader activo. `/__/auth/**` queda excluido del fallback PWA mediante configuración y barrera de artefacto, y la cancelación de popup libera el estado busy con aviso recuperable. Debe desplegarse y repetirse manualmente Google; el Hito 13 y producción permanecen sin aprobación.
