...
- [ ] **Hito 13 - Migrar la autenticación completa a Firebase y sincronizar preferencias.**
  ...
  - **Avance operativo:** la release QA backend `e9c87f25e30f73ce240f08b54d3d14c02c3b32cf` resolvió `LinkTicket`, esquemas discriminados, CSRF restaurable, dominio same-site, capacidades runtime y fixtures/secrets QA. Los subhitos 13.0–13.4 están implementados: sesión JWT/CSRF solo en memoria, dos instancias Firebase, proveedores password/Google/teléfono, onboarding y retornos de correo, cuenta y seguridad, revocaciones, limpieza transversal y preferencias versionadas/realtime. Se retiraron servicios y formularios legacy. `api:lint`, controles QA, build producción/QA, typecheck, 241 unitarias con cobertura, smokes Chromium/Firefox, compact/mobile y baselines visuales Chromium están verdes. La campaña alojada `32729709020` acredita matriz de cinco perfiles, realtime Chromium/Firefox, Hosting, smoke público y restauración Strict de las cuatro identidades password; además restaura `baseline`, libera la lease y sanitiza evidencia. El único bloqueo reproducible es teléfono: preflight `201` y Firebase Phone Auth correctos, seguidos de `403 phone_access_method_not_linked` en `/auth/session` para ambos navegadores. Se abrió `docs/peticiones/restaurar-vinculo-telefono-ficticio-qa.md` y no se repetirán campañas idénticas hasta que backend repare o aclare el fixture. Después quedarán la repetición focalizada/total verde y el smoke Google manual; 13.5, el Hito 13 y producción permanecen sin aprobación.
