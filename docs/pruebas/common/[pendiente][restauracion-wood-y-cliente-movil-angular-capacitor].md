...
- [x] Los defectos se registran con severidad y evidencia sanitizada; H15 termina sin críticos o altos abiertos.
...
- [x] Wood y Mobile no comparten templates o Sass de layout; solo estado y primitives neutrales aprobadas.
- [x] Cambiar 1050/1051, orientación o plegado conserva ruta, filtros, scroll, formulario, borrador y selección editable.
- [x] Solo existe una vista y un conjunto de suscripciones/llamadas por feature.
- [x] Administración se oculta y su guard rechaza Mobile web y Android.
- [x] Las rutas canónicas vigentes siguen resolviendo; no reaparecen aliases `add*`, `update*` o chat retirados.
...
- [x] 1440x900, 1920x1080 y 2560x1080 no presentan overflow, líneas de lectura excesivas ni paneles estirados.
- [x] Bootstrap no se extiende y los estilos Mobile no alteran la cascada Wood en las superficies restauradas de H2-H4.
- [x] Overlays Material, foco, teclado y modales mantienen legibilidad y operación.
...
- [x] 360x800, 390x844, 600x960, 800x1280 y 1050x900 funcionan en portrait/landscape sin overflow global.
- [x] Safe areas y teclado virtual no tapan navegación, acciones ni compositores.
- [x] PWA instala, actualiza mediante el ciclo controlado y no cachea API, imágenes privadas, tokens o datos de cuenta. El artefacto y la campaña alojada final verifican scopes, caché y actualización; el Honor aplicó versiones sucesivas sin quedar bloqueado en un shell antiguo.
- [x] Sin red muestra el shell explicativo y no presenta mutaciones como guardadas.
...
- [x] Home, login, registro, onboarding, recuperación, reset y verificación son operables en Wood y Mobile.
- [x] Autofill, gestores de contraseña, labels, errores y teclado virtual no rompen formularios.
- [x] Access JWT, custom token e ID tokens no aparecen en Web Storage, logs, trazas o artefactos.
- [x] Refresh/CSRF restaura sesión tras recarga; red `0`/503 no fuerza logout y un 401 revocado sí lo hace.
- [x] Renovaciones simultáneas entre pestañas no provocan replay y cada petición se reintenta como máximo una vez.
- [x] Guards esperan inicialización y no muestran rutas privadas/públicas antes de tiempo.
- [x] Password, Google y teléfono cubren alta permitida, login, onboarding, vinculación, reautenticación y errores.
- [x] Cancelar popup/flujo nativo libera el loader y permite reintentar sin recargar.
- [x] Google con correo distinto exige confirmación explícita y conserva la misma cuenta SQL.
- [x] Teléfono ejecuta preflight, conserva intento/código solo durante el flujo y no registra ni actúa como MFA.
- [x] Cuenta y seguridad lista métodos/sesiones, impide retirar el último recuperable y refleja revocaciones.
- [x] Logout limpia Firebase, sockets, presencia, push y stores sin estados parciales.
- [x] El UID canónico de aplicación es siempre `libros:<id_usuario>`.

## Recorridos de producto

- [x] Biblioteca conserva búsqueda, filtros, disponibilidad, scroll y apertura de libro.
- [x] Catálogo conserva detalle, colección, puntuación, reseña y peticiones.
- [x] Autores, universos, sagas, antologías y libros permiten CRUD completo, filtros, paginación e imágenes.
- [x] Las altas actualizan índices y listados sin recarga manual.
- [x] Perfil, políticas, notificaciones, seguridad y estadísticas funcionan en ambas presentaciones.
- [x] Libro permite crear partes, capítulos e interludios y mantiene su índice actualizado.
- [x] Capítulo nuevo con escena aceptable sin personajes se guarda; uno existente aplica validación estricta.
- [x] Autosave termina o bloquea navegación antes de abandonar una ruta.
- [x] Página inicial/final se sincronizan en blur.
- [x] Keywords usan debounce y permiten espacios/puntuación posteriores.
- [x] RTF conserva la selección al usar overlays y toolbars.
- [x] Personajes se asignan sin drag, quedan alfabéticos y el cambio narrativo de nombre conserva apodo.
- [x] Organizaciones, eventos, localizaciones, conceptos, citas, relaciones y entradas tienen CRUD completo.
- [x] Comunidad conserva navegación, relaciones, actividad, perfiles y clubes.
- [x] Chat usa una columna en compact, maestro-detalle en medium y no duplica mensajes/realtime.
- [x] Notificaciones no tapan navegación ni acciones esenciales.
- [x] Backup desktop confirma, evita duplicados y no filtra datos en UI, URL, logs o evidencia.
...
- [x] `npm audit --audit-level=low`, `npm run api:lint`, controles QA, build producción/QA y unitarias con cobertura pasan.
- [x] Playwright Chromium/Firefox cubre Wood 1440/1920/2560 y Mobile 360/390/600/800/1050/1051.
- [x] WebKit permanece en smoke donde aporta señal; PWA/Service Worker se valida en Chromium.
- [x] Axe, teclado, foco, contraste, modales y reduced motion pasan con criterio WCAG 2.2 AA pragmático.
- [x] Seguridad cubre autorización, IDOR, XSS, storage, CORS/CSP, 429 y errores recuperables.
- [x] Realtime cubre duplicados, desorden, desconexión, reconexión y reconciliación REST.
- [x] Campaña QA real usa lease, reset, cleanup, escaneo de secretos y restaura baseline.
- [x] La matriz determinista y el Android físico sustituyen deliberadamente al emulador, según la decisión cerrada en el Hito 5.
- [x] Android físico cubre Google, un teléfono autorizado, push, app links, teclado, orientación y background.
- [x] Evidencias visuales y de rendimiento se revisan y quedan sanitizadas.
- [x] CI, preview/producción, campaña manual y nocturna quedan estables y sin jobs obsoletos en cola.
