...
- El Hito 13 tiene implementados 13.0–13.4 y la aceptación automatizada de 13.5 está verde en `32734302486`. El smoke Google real completó OAuth/normas pero Firefox conservó `[DEFAULT]` porque un Angular Service Worker huérfano servía `chunk-OK5TS2FP.js`, ya ausente de Hosting; la corrección `libros-canonical-session` sí estaba desplegada. `build:qa` genera ahora un artefacto optimizado con hashes, `ngsw.json` y worker, Hosting evita cachear shell/manifiesto y el workflow incorpora una barrera PWA más un smoke de registro real. Build, 21 controles QA y typecheck están verdes. Falta desplegar, repetir Google y confirmar explícitamente antes del visto bueno o producción. QA integral continúa en el Hito 15.
