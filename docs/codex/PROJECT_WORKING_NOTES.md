...
- Continuar por el Hito 11 de `docs/roadmaps/common/ROADMAP_ACTIVO_adaptacion-responsive-multidispositivo.md`: integrar portada y autenticación en light/dark y exponer allí el selector de tema. El Hito 10 queda cerrado: perfil, estadísticas, comunidad, chat, notificaciones y administración ya están adaptados. `_modern-social.sass` y `_modern-admin.sass` concentran patrones compartidos sin emisión global; administración permanece exclusiva de desktop con puntero preciso y limita el conjunto a `1900px`. El centro de notificaciones compacto ocupa el área entre app bar y bottom navigation; no restaurar `backdrop-filter` en `.library-appbar`, porque vuelve a contener y recortar el panel `fixed`. Wood conserva su presentación de escritorio. Los avisos de budget Sass de perfil y del editor narrativo compartido están documentados; no subir budgets y extraer solo patrones con consumidores reales. Material Icons deberá hacerse autosuficiente para offline en el Hito 12. QA integral quedó finalizado por consolidación en `docs/roadmaps/qa/ROADMAP_FINALIZADO_qa-integral-front.md`; sus pendientes están absorbidos por el último hito activo y las campañas contractuales aceptadas no deben repetirse.
