...
- Hitos 0-9 están cerrados y el Hito 10 está en curso. El shell del libro ya tiene appbar, índice overlay/side, hoja de acciones y editor de partes/interludios Mobile propios alrededor de una única instancia del `router-outlet`; búsqueda y estadísticas también tienen vistas Mobile completas sin modificar sus controladores ni Apex de Wood. Continuar por capítulos/escenas y después entidades narrativas táctiles, protegiendo autosave, selección RTF y borradores al cruzar `1050/1051`. La flag sigue apagada fuera de la previsualización local `localStorage['book-front:mobile-presentation-preview']='true'` y Capacitor. La revisión Playwright autenticada de las superficies Mobile se realizará al activar la flag en QA durante H12. El spike Android queda aprobado en `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`; H13/H14 conservan firma, configuración productiva y distribución.
