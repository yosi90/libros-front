...
- Hitos 0-10 están cerrados y el Hito 11 está en curso. El espacio de libro ya separa shell, índice, búsqueda, estadísticas, capítulos, escenas y las seis familias de entidades narrativas en vistas Mobile sobre sus controladores actuales. `ChapterComponent` conserva una única instancia de formularios/autosave y reutiliza el núcleo RTF; la vista parametrizada de entidades conserva altas/ediciones, renombrado narrativo, apodos, entradas, participantes y relaciones. Continuar por comunidad, clubes, mensajería y notificaciones sociales, vigilando que la sustitución de vistas no duplique listeners realtime. La flag sigue apagada fuera de la previsualización local `localStorage['book-front:mobile-presentation-preview']='true'` y Capacitor. La revisión Playwright autenticada de las superficies Mobile se realizará al activar la flag en QA durante H12. El spike Android queda aprobado en `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`; H13/H14 conservan firma, configuración productiva y distribución.
