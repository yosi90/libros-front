...
- Hitos 0-10 están cerrados y el Hito 11 está en curso. El espacio de libro ya separa shell, consulta y narrativa en vistas Mobile. En comunidad, el shell social conserva un único `router-outlet`, resumen y relaciones ya tienen árboles Mobile propios y los controladores siguen siendo dueños de cargas, contadores y realtime. Continuar por la fachada unificada `CommunityComponent` (personas, actividad y clubes), después perfil público, detalle de club, chat/conversación y notificaciones, sin duplicar listeners. La flag sigue apagada fuera de la previsualización local `localStorage['book-front:mobile-presentation-preview']='true'` y Capacitor. La revisión Playwright autenticada se realizará al activar la flag en QA durante H12. El spike Android queda aprobado en `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`; H13/H14 conservan firma, configuración productiva y distribución.
