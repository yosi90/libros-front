...
- Hitos 0-6 están cerrados. El Hito 7 está en progreso: selector seguro, shell público y Home/Login ya tienen vistas Wood/Mobile independientes; faltan registro, onboarding, recuperación, verificación y cierre de App Links. Capacitor fuerza Mobile; en navegador la flag sigue apagada y la revisión de las rutas reales solo puede activarse localmente con `localStorage['book-front:mobile-presentation-preview']='true'`. No habilitar esta llave en QA/producción ni convertirla en query param. H6 fijó tokens/primitives en `src/assets/css/mobile/`, componentes base en `src/app/components/mobile/ui/` y cinco referencias en el laboratorio local protegido `/__mobile-design/:screen`. El spike Android queda aprobado en `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`: en un Honor Magic V3 pasaron contraseña, Google, teléfono ficticio, sesión SQL/cookie opaca, CSRF, UID canónico, realtime, revocación, push FCM real y App Links. La campaña QA Hosting `33159122855` desplegó `67b53d1`, validó la asociación publicada y terminó completamente verde; Android marcó después `qa-libros.yosiftware.es` como `verified` y resolvió de forma implícita reset/verificación hacia la APK. H13/H14 conservan firma, configuración productiva y distribución; no se necesita petición backend de transporte nativo.
