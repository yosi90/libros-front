...
- Esperar la respuesta backend a `docs/peticiones/google-sign-in-firebase-y-vinculacion-cuenta-local.md` y `docs/peticiones/preferencias-interfaz-multidispositivo.md`. Al recibir documentación actualizada, clasificar y mover ambas peticiones según el resultado antes de implementar el Hito 13. Google Sign-In debe conservar credenciales locales, evitar fusión silenciosa por email y mantener `libros:<id_usuario>` como identidad Firebase canónica; no habilitar todavía el proveedor Google ni modificar frontend. El Hito 12 queda cerrado: producción genera PWA instalable, actualizaciones controladas, pantalla offline, caché limitada a shell/assets públicos, Material Icons local, sesión conservada ante estado `0` y Firebase Messaging aislado del Angular Service Worker mediante un scope propio. No añadir caché de API ni escritura offline sin contratos explícitos. Los avisos de budget Sass de perfil y del editor narrativo compartido están documentados; no subir budgets y extraer solo patrones con consumidores reales. QA integral quedó finalizado por consolidación en `docs/roadmaps/qa/ROADMAP_FINALIZADO_qa-integral-front.md`; sus pendientes están absorbidos por el último hito activo y las campañas contractuales aceptadas no deben repetirse.
