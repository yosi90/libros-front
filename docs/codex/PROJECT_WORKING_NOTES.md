...
- El Hito 13 tiene implementados 13.0–13.5 y producción publica Firebase sobre la release limpia `315ae4b06aa7aadab96dccba2972bb6306207157`. El contrato cerrado para confirmar un Google con correo distinto está integrado en frontend: solo se muestran los dos valores enmascarados, el ID token permanece en memoria, cancelar lo descarta y aceptar reintenta con `ConfirmEmailMismatch=true`. Unitarias 252/252, `api:lint`, tipos E2E y builds producción/QA están verdes. Pendiente: desplegar esta entrega y repetir el smoke productivo completo antes de autorizar que backend retire `usuarios.password`. QA integral continúa en el Hito 15.
