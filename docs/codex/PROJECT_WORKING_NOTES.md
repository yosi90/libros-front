...
- Hitos 0-10 están cerrados y el Hito 11 está en curso. En comunidad ya tienen árboles Mobile propios el shell, resumen, relaciones y la fachada `CommunityComponent`: personas, feed y clubes conservan todos sus comandos sobre los mismos controladores y listeners realtime. El centro de acceso también separa su presentación sin duplicar bandejas ni peticiones. Continuar por perfil público, detalle de club, chat/conversación y notificaciones. La flag sigue apagada fuera de la previsualización local `localStorage['book-front:mobile-presentation-preview']='true'` y Capacitor. La revisión Playwright autenticada se realizará al activar la flag en QA durante H12. El spike Android queda aprobado en `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`; H13/H14 conservan firma, configuración productiva y distribución.
