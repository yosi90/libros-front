...
- Continuar el Hito 5 conforme a `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`. Hitos 0-4 están cerrados. Capacitor 8.5, Android QA/producción, Firebase Auth 8.4, App 8.1.1 y Push Notifications 8.1.2 ya están incorporados. Android Studio, Java 21, SDK 35/36, herramientas de línea de comandos y variables locales están configurados; 287 unitarias, builds Angular QA/producción, sync/configuración Gradle y `assembleQaDebug` están verdes. La APK QA ya se instaló por depuración inalámbrica en un Honor Magic V3 con Android 16: Angular renderiza, fuerza `native-mobile` con el panel desplegado (`718x748`, DPR 3) y alcanza la API mediante `CapacitorHttp`. Firebase Android QA y su JSON aislado en `src/qa` están validados localmente. Contraseña, Google nativo, cancelación de Credential Manager, teléfono ficticio sin SMS real, onboarding, sesión SQL, cookie jar opaco, recuperación CSRF, restauración y revocación están validados. La revocación física devuelve `/home`, elimina usuario Firebase, invalida CSRF, conserva ausentes JWT/refresh y limpia todas las claves `push-device:*`. Custom token, UID canónico, presencia, Realtime Database y su socket están acreditados mediante eventos QA sanitizados. Push nativo completó permiso, alta, re-registro, guardado de preferencias y recepción física de una entrega FCM real en segundo plano. Backend restauró `baseline` y la petición correspondiente quedó aceptada. Los handlers App Links abren la ruta correcta dirigidos al paquete; el build QA incluye `assetlinks.json`, pero el Hosting público todavía responde `[]`. No declarar cerrado el spike hasta desplegarlo y acreditar la verificación automática del dominio.
