...
- La dirección visual vigente quedó cerrada en `docs/roadmaps/common/ROADMAP_FINALIZADO_restauracion-wood-y-cliente-movil-angular-capacitor.md`: Wood se usa en escritorio/ultrawide y Mobile es un árbol Angular independiente para móvil/plegable/tablet y Capacitor. Light/dark y el selector de tema están retirados.
...
- El roadmap anterior H0-H15 está cerrado. El lector persistente Android queda validado en `1.0.20-qa` (`versionCode 21`, ejecución `33400547517`): conserva raíz y subruta por identidad, sin loader ni red al restaurar dentro del mismo proceso. `1.0.22-qa` (`versionCode 23`, ejecución `33403400971`) deja además Angular Service Worker activo y controlando la raíz nativa; la siguiente APK con cambios frontend debe validar físicamente la barrera y recarga automática. El siguiente foco es ejecutar las tandas visuales Android de `ROADMAP_ACTIVO_lector-persistente-y-pulido-multisoporte.md`. Mobile continúa activo en QA y producción para compact/medium; escritorio y ultrawide conservan Wood desde 1051 CSS px, y Capacitor fuerza `native-mobile`.
