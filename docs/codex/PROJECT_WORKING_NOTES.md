...
- Continuar por el Hito 7 de `docs/roadmaps/common/ROADMAP_ACTIVO_adaptacion-responsive-multidispositivo.md`: adaptar el espacio de trabajo y el índice del libro. El Hito 4 separó la composición editorial wood, exclusiva de escritorio, del shell light/dark adaptativo; el Hito 5 adaptó biblioteca y catálogo; y el Hito 6 adaptó los cinco gestores mediante cards reutilizables, editor enrutado completo cuando no cabe una lista-detalle, formulario lateral cuando hay ancho, persistencia por tipo de filtros/paginación/scroll y contención ultrawide. `ManagerEntityCardComponent`, `object-manager.models.ts` y `ManagerViewStateService` son los contratos reutilizables de esa vertical. Wood no debe usarse como plantilla responsive ni recibir el rediseño moderno salvo correcciones funcionales o transversales. La portada y autenticación consumirán el selector de tema en el Hito 11; Material Icons deberá hacerse autosuficiente para offline en el Hito 12. QA integral quedó finalizado por consolidación en `docs/roadmaps/qa/ROADMAP_FINALIZADO_qa-integral-front.md`; sus pendientes están absorbidos por el último hito activo y las campañas contractuales aceptadas no deben repetirse.
