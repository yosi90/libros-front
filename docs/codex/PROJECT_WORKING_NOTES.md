...
- El Hito 13 tiene implementados 13.0–13.4 y la campaña automática completa `32742203290` está verde. El smoke Google real posterior mostró que el Angular Service Worker sustituía `/__/auth/handler` por la SPA y abría `/home` dentro del popup; al cerrarlo, el login podía mantener el loader. `/__/auth/**` queda excluido de las navegaciones PWA mediante configuración y barrera de artefacto, y la cancelación libera el estado busy con aviso recuperable. Debe desplegarse y repetirse manualmente Google antes de marcar 13.5 o enviar visto bueno. QA integral continúa en el Hito 15.
