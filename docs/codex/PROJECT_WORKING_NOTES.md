...
- Hitos 0-11 están cerrados y H12 está en validación alojada. Mobile está activado en `environment.qa.ts`, continúa apagado en producción y Capacitor sigue forzando `native-mobile`. Light/dark, su selector, servicios y Sass condicional ya fueron retirados; los valores históricos solo sobreviven en el contrato de API. La matriz local cubre 320-3440 px, anchos contractuales 360/390/600/800/1050/1051, orientación, plegable y conservación de formulario. Hosting QA ya sirve Mobile: la primera campaña desplegó correctamente pero un smoke esperaba el copy Wood del botón Google; el test corregido pasa en los tres motores. `PwaLifecycleService` solicita una comprobación al arrancar y mantiene la activación bajo acción explícita. `npm run qa:android-browser` valida Chrome físico por CDP sin credenciales; el Honor Magic V3 abierto pasó medium a 718x652 CSS px. El siguiente foco es relanzar la campaña completa y verificar el dispositivo plegado antes de cerrar H12. Comunidad, perfiles, clubes, mensajería y notificaciones tienen árboles Mobile propios sobre los mismos controladores, stores y listeners realtime. En conversación, `MobileChatConversationViewComponent` transfiere su `#messageList` al controlador porque `ViewChild` no atraviesa componentes; preservar ese puente al tocar scroll, carga anterior o mensajes correlacionados. El spike Android queda aprobado en `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`; H13/H14 conservan firma, configuración productiva y distribución.
