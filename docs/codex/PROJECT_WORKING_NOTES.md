...
- El Hito 13 tiene implementados 13.0–13.5 y producción publica Firebase sobre la release limpia `315ae4b06aa7aadab96dccba2972bb6306207157`. El contrato para confirmar un Google con correo distinto está integrado y su smoke productivo completo fue correcto: solo se muestran valores enmascarados, el token queda en memoria y Google abre la misma cuenta conservando datos y preferencias. Se detectó después que `/dashboard/account-security` era demasiado difícil de descubrir; ya se añadió como destino explícito en sidebar moderna y panel “Más”, pendiente únicamente de desplegar y comprobar su visibilidad antes del visto bueno final para retirar `usuarios.password`. Unitarias 252/252 y build producción están verdes. QA integral continúa en el Hito 15.
