...
- El Hito 13 tiene implementados 13.0–13.4 y la aceptación automatizada de 13.5 está verde en `32734302486`. El smoke Google real expuso un Angular Service Worker huérfano que servía un chunk antiguo pese a estar desplegada la corrección `libros-canonical-session`. Tras limitar el worker QA al Hosting canónico, el run `32740929813` superó gate, integración local, despliegue, smoke PWA alojado y login telefónico; Firefox falló al cargar mediante el worker `fondo_menu.png` y `escritorio_login.png`, con contenido truncado/codificación inválida, y el cleanup quedó verde. Los fondos, animaciones pesadas y recursos descartados no deben formar parte del cache PWA; el shell, fuentes, iconos y recursos funcionales pequeños permanecen versionados. No enviar visto bueno ni autorizar producción. QA integral continúa en el Hito 15.
