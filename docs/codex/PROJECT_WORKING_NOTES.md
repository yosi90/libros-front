...
- El Hito 13 tiene implementados 13.0–13.4 y la aceptación automatizada de 13.5 está verde en la campaña `32734302486` contra backend `16090b4ce05eda9307da29679bdfc9cb6e1616ee` y dataset `2026.08.4`. El smoke Google real completó OAuth y normas, pero descubrió al regresar a Biblioteca que `FirebaseSessionService` buscaba `[DEFAULT]` tras crear primero la app nombrada de proveedor. La corrección usa la app canónica nombrada `libros-canonical-session`, está reproducida unitariamente y deja 242 unitarias/build QA verdes. Falta desplegarla, repetir el regreso Google y obtener la confirmación explícita del propietario antes de enviar el visto bueno o autorizar producción. QA integral continúa en el Hito 15 como regresión final.
