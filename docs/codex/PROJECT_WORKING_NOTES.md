...
- Hitos 0-7 están cerrados y el Hito 8 está en curso. El shell autenticado Mobile ya tiene appbar, navegación inferior/rail y panel Más propios, alrededor de una única instancia del `router-outlet`; la biblioteca ya usa una vista Mobile independiente conectada al estado/comandos existentes. Continuar por catálogo/ficha y cuenta, sin duplicar servicios ni reintroducir el HTML/Sass adaptativo retirado. La flag sigue apagada fuera de la previsualización local `localStorage['book-front:mobile-presentation-preview']='true'` y Capacitor. La revisión visual de superficies autenticadas requiere sesión QA real. El spike Android queda aprobado en `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`; H13/H14 conservan firma, configuración productiva y distribución.
