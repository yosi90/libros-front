...
- Hitos 0-8 están cerrados y el Hito 9 es el siguiente foco. El shell autenticado Mobile ya tiene appbar, navegación inferior/rail y panel Más propios, alrededor de una única instancia del `router-outlet`; biblioteca, catálogo, ficha pública, Cuenta y seguridad, estadísticas, perfil y preferencias usan vistas Mobile independientes conectadas a sus controladores existentes. Adaptar ahora los gestores de autores, universos, sagas, antologías y libros mediante listas/cards y maestro-detalle, sin duplicar servicios ni reintroducir el HTML/Sass adaptativo retirado. La flag sigue apagada fuera de la previsualización local `localStorage['book-front:mobile-presentation-preview']='true'` y Capacitor. La revisión Playwright autenticada de las superficies Mobile se realizará al activar la flag en QA durante H12. El spike Android queda aprobado en `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`; H13/H14 conservan firma, configuración productiva y distribución.
