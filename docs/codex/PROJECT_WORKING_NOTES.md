...
- El Hito 13 tiene implementados 13.0–13.5: la campaña automática completa `32746025039` está verde y el smoke Google real completó OAuth tras migrar con `Ctrl+F5` las dos ventanas que aún conservaban el worker defectuoso anterior. Una comprobación nueva con navegador controlado por el worker alojado confirma que `/__/auth/handler` entrega `handler.js` y nunca la SPA; queda incorporada como regresión permanente. La cancelación del popup también libera el loader. El propietario autorizó producción y el handoff está en `docs/peticiones/autorizar-corte-productivo-autenticacion-firebase.md`; esperar respuesta backend y ejecutar después solo un smoke productivo no destructivo. QA integral continúa en el Hito 15.
