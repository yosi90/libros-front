...
- El Hito 13 tiene implementados 13.0–13.5 y producción publica correctamente Firebase. Backend desplegó sobre la release limpia `a7c1a8593015278575ae86b2c99a3fad09d1fa90` la confirmación para vincular Google a una cuenta Outlook, pero OpenAPI no tipa las dos direcciones enmascaradas de `details` y la guía conserva el SHA anterior. No adivinar claves ni registrar la respuesta real; `docs/peticiones/completar-contrato-confirmacion-google-correo-distinto.md` solicita únicamente el schema cerrado, ejemplo sanitizado y release efectiva. Backend debe conservar `usuarios.password` hasta implementar el front y repetir el smoke. QA integral continúa en el Hito 15.
