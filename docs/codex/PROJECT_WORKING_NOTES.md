...
- El Hito 13 tiene implementados 13.0–13.4 y la aceptación automatizada de 13.5 está verde en `32734302486`. El smoke Google real expuso un Angular Service Worker huérfano que servía un chunk antiguo pese a estar desplegada la corrección `libros-canonical-session`. El run `32737918743` verificó el nuevo artefacto PWA pero no desplegó: el worker se registró en la integración local y Firefox falló durante reset realtime con `NS_ERROR_INVALID_CONTENT_ENCODING`; cleanup quedó verde. El registro QA se ha limitado después a `qa-libros.yosiftware.es` y una barrera Playwright confirma que permanece desactivado en `127.0.0.1`; 244 unitarias, build QA, artefacto PWA, typecheck y smoke público Firefox local quedan verdes antes de repetir la campaña. No enviar visto bueno ni autorizar producción. QA integral continúa en el Hito 15.
