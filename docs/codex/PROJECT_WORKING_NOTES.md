...
- Hitos 0-14 están cerrados. Mobile está activado en QA y producción para compact/medium; escritorio y ultrawide conservan Wood desde 1051 CSS px, y Capacitor fuerza `native-mobile`. Light/dark, su selector, servicios y Sass condicional ya fueron retirados; los valores históricos solo sobreviven en el contrato de API. La campaña Hosting QA `33274806877`, sobre `8042602`, terminó completamente verde: gates, builds, unitarias, navegadores, PWA, despliegue, App Links, sesiones Firebase, superficies Wood/Mobile, WCAG, cleanup, baseline y escaneo de secretos. La activación productiva `33276370003`, sobre `1623cff`, superó `qa:ci`, desplegó Firebase Hosting y quedó acreditada con la matriz pública alojada `30/30` entre 320 y 3440 px. La matriz total cubre los anchos contractuales 360/390/600/800/1050/1051, orientación, plegable simulado y conservación de formulario. `PwaLifecycleService` solicita una comprobación al arrancar y mantiene la activación bajo acción explícita. `npm run qa:android-browser` valida Chrome físico por CDP sin credenciales; el Honor Magic V3 pasó abierto en Mobile medium a 718x652 CSS px y plegado en Mobile compact a 353x703 CSS px, sin overflow, árbol Wood ni colisión de scopes. Comunidad, perfiles, clubes, mensajería y notificaciones tienen árboles Mobile propios sobre los mismos controladores, stores y listeners realtime. En conversación, `MobileChatConversationViewComponent` transfiere su `#messageList` al controlador porque `ViewChild` no atraviesa componentes; preservar ese puente al tocar scroll, carga anterior o mensajes correlacionados. El spike Android queda aprobado en `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`; H13 y H14 quedan acreditados en `ANDROID_INTEGRATION_HITO_13.md` y `ANDROID_DISTRIBUTION_HITO_14.md`. La clave release está en GitHub Actions y el propietario confirmó dos copias offline; Firebase y App Links producción usan su SHA. El único foco restante es H15: campaña integral, instalación/actualización física y, solo después de verde, primera GitHub Release productiva.
