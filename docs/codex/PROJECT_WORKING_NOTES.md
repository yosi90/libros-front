...
- El Hito 13 tiene implementados 13.0–13.4 y la aceptación automatizada de 13.5 está verde en `32734302486`. El smoke Google real expuso un Angular Service Worker huérfano que servía un chunk antiguo pese a estar desplegada la corrección `libros-canonical-session`. Tras limitar el worker QA al Hosting canónico, el run `32739468999` superó gate, integración local, despliegue y smoke PWA alojado. Falló únicamente al leer con Playwright el body de `/auth/session` después de que el login telefónico Firefox ya hubiera navegado al dashboard; el cleanup quedó verde. La prueba debe aceptar el estado HTTP y el estado autenticado visible sin depender de un body descartado por la navegación. No enviar visto bueno ni autorizar producción. QA integral continúa en el Hito 15.
