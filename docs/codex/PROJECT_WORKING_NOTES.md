...
- El Hito 13 tiene implementados 13.0–13.4. La campaña alojada `32729709020` dejó verdes cinco perfiles/realtime Chromium-Firefox, Hosting, smoke público, password/refresh Strict y cleanup `baseline`, pero detectó `403 phone_access_method_not_linked` en teléfono. Backend respondió mediante la release `16090b4ce05eda9307da29679bdfc9cb6e1616ee` y el dataset `2026.08.4`: añadió el alias `auth.phone.member-a`, fijó el UID `libros-auth:phone:900003` y restauró su vínculo HMAC con `user.member-a`. La validación focal `32733090045` confirma el arreglo con teléfono `2/2`. La campaña total `32733418202` superó gate, build y los escenarios hasta que el keepalive coincidió con un reset de Firefox: `/qa/status` devolvió transitoriamente `ContinueCampaign=blocked` con la única razón `reset_in_progress` y el cliente lo trató erróneamente como bloqueo definitivo. El cleanup restauró `baseline`, liberó la lease y sanitizó la evidencia. Debe corregirse esa carrera de control y repetir la campaña total; después quedará el smoke Google manual. No enviar visto bueno ni autorizar producción sin confirmación explícita. QA integral continúa en el Hito 15 como regresión final.
