...
- El Hito 13 tiene implementados 13.0–13.4. La campaña alojada `32729709020` deja verdes cinco perfiles/realtime Chromium-Firefox, Hosting, smoke público, password/refresh Strict y cleanup `baseline`, pero bloquea teléfono: preflight `201` y Firebase Phone Auth correctos terminan en `403 phone_access_method_not_linked` desde `/auth/session` en ambos navegadores. La petición pendiente `docs/peticiones/restaurar-vinculo-telefono-ficticio-qa.md` exige restaurar el vínculo determinista del fixture. No repetir campañas completas idénticas hasta cambio backend, ni enviar visto bueno o autorizar producción; después quedarán la aceptación verde y el smoke Google manual. QA integral continúa en el Hito 15 como regresión final.
