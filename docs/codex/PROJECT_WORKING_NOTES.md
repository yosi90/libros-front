...
- Hitos 0-9 están cerrados y el Hito 10 está en curso. Shell, índice, búsqueda, estadísticas, capítulos y escenas ya tienen vistas Mobile independientes sobre sus controladores actuales. `ChapterComponent` conserva una única instancia de formularios/autosave y reutiliza el núcleo RTF; su piel táctil se activa por `data-presentation-active`, no por los temas retirados. Continuar por personajes y el placeholder unificado de organizaciones, eventos, localizaciones, conceptos y citas, sin perder relaciones, apodos ni entradas. La flag sigue apagada fuera de la previsualización local `localStorage['book-front:mobile-presentation-preview']='true'` y Capacitor. La revisión Playwright autenticada de las superficies Mobile se realizará al activar la flag en QA durante H12. El spike Android queda aprobado en `docs/roadmaps/common/ANDROID_SPIKE_HITO_5.md`; H13/H14 conservan firma, configuración productiva y distribución.
